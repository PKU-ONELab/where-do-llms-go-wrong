...
3. **Artifact inspection:** download the Hugging Face dataset and inspect `content_pairs/` first; this is the reusable before/after perturbation benchmark. The redundant perturbed-only view is intentionally not a separate public surface because it duplicates `content_pairs.content_after`.
