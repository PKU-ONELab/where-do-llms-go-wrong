...
def parse_file_info(file_name: str) -> dict:
    """Parses the file name to extract file type, prompt setting, and perturbation type."""
...
# ICLR 2024 acceptance stats (total=5779):
#  "Reject": 3519 (0.61 cumulative)
#  "Accept (poster)": 1808 => next cumulative = 0.61 + (1808/5779)= ~0.92
#  "Accept (spotlight)": 366 => next cum. ~0.98
#  "Accept (oral)": 86 => next cum. ~1.0

DECISION_MAPPING = {
    "Reject": 0.0,
    "Accept as Poster": 0.61,
    "Accept as Spotlight": 0.92,
    "Accept as Oral": 1.0
}
...
def perform_and_print_tests(baseline_df, perturb_df,
    ...
    using a field-specific delta from FIELD_DELTA_MAP. Prints out the p-values for each test.
    """
        ...
        for prompt_setting in prompt_settings:
            ...
            for perturbation_type in perturbation_types:
                ...
                for score_col in common_cols:
                    ...
                    # Print p-values for Wilcoxon tests and TOST
                    print(f"    Wilcoxon p-value (greater): {results['wilcoxon_p_greater']:.3f}")
                    print(f"    Wilcoxon p-value (less): {results['wilcoxon_p_less']:.3f}")
                    if results['TOST_p_lower'] is not None and results['TOST_p_upper'] is not None:
                        print(f"    TOST p-value (lower): {results['TOST_p_lower']:.3f}")
                        print(f"    TOST p-value (upper): {results['TOST_p_upper']:.3f}")
