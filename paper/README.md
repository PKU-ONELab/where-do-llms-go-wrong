# Paper

**Where Do LLMs Go Wrong? Diagnosing Automated Peer Review via Aspect-Guided Multi-Level Perturbation**
Jiatao Li, Yanheng Li, Xinyu Hu, Mingqi Gao, Xiaojun Wan. CIKM 2025.

- DOI: https://doi.org/10.1145/3746252.3761274
- Dataset: https://huggingface.co/datasets/PKU-ONELab/ai-reviewer-diagnostic-data
- Citation: [`../CITATION.bib`](../CITATION.bib), [`../CITATION.cff`](../CITATION.cff)

The publisher version is linked rather than redistributed here.
